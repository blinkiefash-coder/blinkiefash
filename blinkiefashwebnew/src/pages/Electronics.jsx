...
// NOTE: dedicated hero banner artwork for Electronics doesn't exist yet.
// Reusing the single electronics banner for all three hero slides as a
// placeholder — swap these three imports for real desktop/mobile banner
// assets (e.g. electronicsBanner1/2/3, electronicsMobileBanner1/2/3) the
// same way the Women page does with womenBanner1/2/3.
...
// the Electronics New Arrivals pool. Each page pulls up to 100 products, so
// 4 pages covers up to 400 of the most recently added electronics products
...
const ELECTRONICS_CATEGORY_FALLBACK = [
  { label: "Phones", icon: MdPhoneIphone },
  { label: "Laptops", icon: MdLaptop },
  { label: "Audio", icon: MdHeadphones },
  { label: "Wearables", icon: MdWatch },
  { label: "Televisions", icon: MdTv },
  { label: "Cameras", icon: MdCameraAlt },
  { label: "Gaming", icon: MdSportsEsports },
  { label: "Tablets", icon: MdTabletMac },
  { label: "Chargers & Cables", icon: MdPower },
...
const HERO_SLIDES = [
  { image: electronicsBanner, mobileImage: null, tag: "Latest smartphones & gadgets" },
  { image: electronicsBanner, mobileImage: null, tag: "Top audio & wearables deals" },
  { image: electronicsBanner, mobileImage: null, tag: "New arrivals in tech" },
];

let electronicsPageCache = null;
...
export default function Electronics() {
  ...
  const [products, setProducts] = useState(() => electronicsPageCache?.products ?? []);
  const [newArrivals, setNewArrivals] = useState(() => electronicsPageCache?.newArrivals ?? []);
  const [deals, setDeals] = useState(() => electronicsPageCache?.deals ?? []);
  const [productsLoading, setProductsLoading] = useState(!electronicsPageCache);
  const [newArrivalsLoading, setNewArrivalsLoading] = useState(!electronicsPageCache);
  const [rootId, setRootId] = useState(() => electronicsPageCache?.rootId ?? null);
  const [subcats, setSubcats] = useState(() => electronicsPageCache?.subcats ?? []);
  const [categoryResolved, setCategoryResolved] = useState(() => Boolean(electronicsPageCache));
  const [brands, setBrands] = useState(() => electronicsPageCache?.brands ?? []);
  ...
  useEffect(() => {
    if (electronicsPageCache) return undefined;
    let cancelled = false;
    ...
    (async () => {
      ...
      const resolvedRootId = rootIdForAny(allCats, ["Electronics", "Electronic", "Gadgets"]);
  ...
  useEffect(() => {
    if (electronicsPageCache?.brands?.length) return undefined;
    let cancelled = false;
    ...
    (async () => {
      try {
        ...
        if (!cancelled) {
          ...
          electronicsPageCache = { ...(electronicsPageCache || {}), brands: nextBrands };
        }
      } catch {
        if (!cancelled) {
          setBrands(TOP_BRANDS_FALLBACK);
          electronicsPageCache = { ...(electronicsPageCache || {}), brands: TOP_BRANDS_FALLBACK };
        }
      }
    })();
  ...
  useEffect(() => {
    ...
    if (electronicsPageCache?.products) return;
    let cancelled = false;
    ...
    (async () => {
      ...
      if (!found.length) {
        try {
          const bySearch = await getProducts({ search: "electronics", limit: 30 });
      ...
      if (!dealList.length) {
        try {
          const fallbackDeals = await getProducts({
            ...
            search: rootId ? undefined : "electronics",
            sort: "newest",
            limit: 12,
          });
  ...
  // for "New on Blinkiefash"), scoped to the Electronics category only.
  // This pages through `sort: "newest"` results instead of reusing
  // whatever "All Electronics Picks" happened to fetch, so it reflects
  // products that were actually added most recently.
  useEffect(() => {
    if (!categoryResolved) return;
    if (electronicsPageCache?.newArrivals) return;
    let cancelled = false;
    ...
    (async () => {
      ...
      try {
        ...
        electronicsPageCache = { ...(electronicsPageCache || {}), newArrivals: normalizedNewArrivals };
      } catch {
    ...
    (async () => {
      ...
      try {
        ...
        const res = await getProducts({
          ...
          search: categoryId ? undefined : "electronics",
          sort: "newest",
  ...
  const electronicsScopedShopUrl = useCallback(
    (opts = {}) => {
      ...
      if (!params.has("category_id")) {
        ...
        if (!lower.includes("electronic") && !lower.includes("gadget")) {
          search = search ? `electronics ${search}` : "electronics";
        } else if (!search) {
          search = "electronics";
        }
      }
      ...
      return qs ? `/shop?${qs}` : "/shop?search=electronics";
    },
    [rootId]
  );
    ...
    if (subcats.length) {
      return subcats.map((cat) => {
        ...
        return {
          ...
          label: cat.name,
          image,
          to: electronicsScopedShopUrl({ categoryId: cat.id }),
        };
      });
    }
    return ELECTRONICS_CATEGORY_FALLBACK.map((item) => {
      ...
      return {
        ...
        to: match
          ? electronicsScopedShopUrl({ categoryId: match.id })
          : electronicsScopedShopUrl({ search: item.label }),
      };
    });
  }, [subcats, electronicsScopedShopUrl, findSubcatByLabel]);
  ...
  // Only brands that appear on electronics products
  const electronicsBrands = useMemo(() => {
  ...
  const heroDestination = heroIndex === 2
    ? electronicsScopedShopUrl({ sort: "newest", newArrivals: true })
    : electronicsScopedShopUrl();

  const filtersPanel = (
    <section className="electronics-filters-panel" role="dialog" aria-label="Electronics filters">
      <div className="electronics-filters-panel-header">
        <h3>Filters</h3>
        <div className="electronics-filters-panel-actions">
          {activeFilterCount > 0 ? (
            <button type="button" className="electronics-filters-clear" onClick={clearAllFilters}>
              Clear All
            </button>
          ) : null}
          <button
            type="button"
            className="electronics-filters-close"
      ...
      <div className="electronics-filter-col">
        <h4>Brand</h4>
        <input
        ...
        />
        <div className="electronics-filter-list">
      ...
      <div className="electronics-filter-col">
        <h4>Color</h4>
        <div className="electronics-filter-list electronics-filter-swatches">
          {COLORS.map(([name, hex]) => {
            const checked = activeColor.includes(name.toLowerCase()) || activeColor.includes(name);
            return (
              <label key={name} className={`electronics-swatch-label${checked ? " checked" : ""}`}>
                <input type="checkbox" checked={checked} onChange={() => toggleColorFilter(name)} />
                <span className="electronics-swatch-dot" style={{ background: hex }} />
                <span>{name}</span>
              </label>
            );
          })}
      ...
      <div className="electronics-filter-col">
        ...
        <div className="electronics-filter-discount-chips">
          {DISCOUNT_BUCKETS.map((value) => (
            <button
              key={value}
              type="button"
              className={`electronics-filter-chip-item${minDiscount === value ? " active" : ""}`}
      ...
      <div className="electronics-filters-footer">
        <button type="button" className="electronics-filters-apply" onClick={applyFilters}>
          Apply Filters
        </button>
      </div>
    </section>
  ...
  return (
    <div className={`catalog-page electronics-page${!categoryResolved ? " electronics-loading" : ""}`}>
      {!categoryResolved && <Loader overlay />}
      <PageSEO
        title="Electronics — Phones, Laptops, Audio & More"
        description="Shop phones, laptops, audio, wearables and cameras at Blinkiefash — delivered fast across Odisha."
        path="/electronics"
      />

      <div className="electronics-top-strip">
        <div className="electronics-top-strip-inner">
          {TOP_STRIP_ITEMS.map((item) => (
            <span className="electronics-top-strip-item" key={item.label}>
              <item.icon />
      ...
      <main className="electronics-main">
        ...
        >
          <button
            type="button"
            className="electronics-hero-arrow prev"
          ...
          <button type="button" className="electronics-hero-media-btn" onClick={() => navigate(heroDestination)}>
            <picture>
              {slide.mobileImage ? (
                <source media="(max-width: 767px)" srcSet={slide.mobileImage} />
              ) : null}
              <img src={slide.image} alt={slide.tag} className="electronics-hero-img" draggable={false} />
            </picture>
          </button>

          <button
            type="button"
            className="electronics-hero-arrow next"
          ...
          <div className="electronics-hero-dots">
            {HERO_SLIDES.map((s, idx) => (
              <button
                key={s.tag}
                type="button"
                className={`electronics-hero-dot${idx === heroIndex ? " active" : ""}`}
        ...
        {topDeals.length > 0 && (
          <section className="section electronics-picks-section">
            <div className="hp-shead">
              ...
              <div className="electronics-section-actions">
                <button
                  type="button"
                ...
                <button
                  type="button"
                  className={`electronics-filter-btn${filterOpen || activeFilterCount ? " is-active" : ""}`}
                  onClick={() => {
            ...
            <ProductRail list={applyProductFilters(topDeals)} railRef={dealsRef} keyPrefix="electronics-deal" />
          </section>
        )}

        {/* ========== BRANDS YOU WILL LOVE (under Deals, electronics-only brands) ========== */}
        {electronicsBrands.length > 0 && (
          <section className="section hp-shop-brands-section electronics-brands-section" aria-label="Brands you will love">
            <div className="hp-shead">
              <div className="hp-shead-title-group">
                <div className="hp-shead-title-wrap">
              ...
              <button type="button" className="hp-shead-action" onClick={() => navigate(electronicsScopedShopUrl())}>
                View All <MdChevronRight />
              </button>
            </div>
            ...
            <div className="hp-deals-wrap hp-shop-brands-wrap">
              ...
              <div className="hp-shop-brands-grid" role="list" ref={shopBrandsRef}>
                {electronicsBrands.map((brand, idx) => {
        ...
        {/* All Electronics Picks */}
        <section className="section electronics-picks-section">
          <div className="hp-section-head">
            <h2>ALL ELECTRONICS PICKS ⚡</h2>
            <button type="button" onClick={() => navigate(electronicsScopedShopUrl())}>
              View All <MdChevronRight />
            </button>
          </div>
          {productsLoading ? (
            ...
            <ProductRail list={applyProductFilters(products)} railRef={trendingRef} keyPrefix="electronics-all" />
          ) : (
            <p className="electronics-empty-state">New electronics are landing soon.</p>
          )}
        </section>

        {/* Categories */}
        <section className="electronics-cat-strip" aria-label="Shop by category">
          <div className="electronics-cat-list">
            {categoryStripItems.map((cat) => (
              <button key={cat.id} type="button" className="electronics-cat-item" onClick={() => navigate(cat.to)}>
                <span className="electronics-cat-icon-wrap">
                  {cat.image ? (
                    <img
                      src={cat.image}
                      alt={cat.label}
                  ...
                  <span className="electronics-cat-fallback-icon" style={cat.image ? { display: "none" } : undefined}>
                    {cat.icon ? <cat.icon /> : <MdGridView />}
                  </span>
                </span>
                <span className="electronics-cat-label">{cat.label}</span>
              </button>
            ))}
          </div>
        </section>

        {/* New arrivals — genuinely newest electronics products (Home-style pool) */}
        <section className="section electronics-picks-section">
          <div className="hp-section-head">
            <h2>NEW ARRIVALS ⚡</h2>
            <button type="button" onClick={() => navigate(electronicsScopedShopUrl({ search: "newest" }))}>
              View All <MdChevronRight />
            </button>
          </div>
          {newArrivalsLoading ? (
            ...
            <ProductRail
              ...
              railRef={arrivalsRef}
              keyPrefix="electronics-new"
              isNew
            />
          ) : (
            <p className="electronics-empty-state">Fresh tech coming soon.</p>
          )}
        </section>

        {/* More to Explore */}
        <section className="section electronics-explore-section" aria-label="More to explore">
          ...
          {exploreProducts.length > 0 ? (
            ...
            <p className="electronics-empty-state">No products in this category yet.</p>
          ) : (
            <div className="hp-explore-grid" role="list">
        ...
        <section className="electronics-trust-strip" aria-label="Why shop with us">
          <div>
            <MdBolt />
            <div>
              <strong>FAST</strong>
