...
// Adjust this to match your real free-delivery rule.
const FREE_DELIVERY_THRESHOLD = 999;

export default function Cart() {
  const navigate = useNavigate();
  const { items, updateQty, removeFromCart, addToCart, subtotal, count } = useCart();
  const { toggleWishlist } = useWishlist();

  const [savedItems, setSavedItems] = useState(() => {
    try {
      const stored = JSON.parse(localStorage.getItem(SAVED_FOR_LATER_KEY) || '[]');
  ...
  const totalMrp = items.reduce(
    (sum, item) => sum + Number(item.mrp ?? item.price ?? 0) * Number(item.qty || 1),
    0
  );
  ...
  return (
    <div className="page cart-page">
      ...
      <div className="cart-header-row">
        <div>
          ...
          <p className="cart-subtitle">
            {items.length} {items.length === 1 ? 'item' : 'items'} • Total {count} {count === 1 ? 'item' : 'items'}
          </p>
        </div>
            ...
            return (
              <article className="cart-item-card" key={key}>
                  ...
                  <div className="cart-item-info">
                    ...
                    {(item.color || item.size) && (
                      <p className="cart-item-variant">
                        ...
                        {item.color && item.size ? <span className="cart-item-variant-sep" /> : null}
                    ...
                    <div className="cart-item-qty">
                      <button
                        type="button"
                        onClick={() => updateQty(key, Math.max(1, item.qty - 1))}
                ...
                <div className="cart-item-actions">
                  <button type="button" className="cart-action" onClick={() => removeFromCart(key)}>
                    <MdDeleteOutline /> Remove
                  </button>
                  <button type="button" className="cart-action" onClick={() => handleSaveForLater(item, key)}>
        ...
        <section className="cart-saved-section">
          ...
          <div className="cart-list">
            {savedItems.map((item) => {
              const key = keyOf(item);
              return (
                <article className="cart-item-card cart-item-card-saved" key={key}>
                  <div className="cart-item-top">
                    <div className="cart-item-media">
                      {item.image ? <img src={item.image} alt={item.name} /> : null}
                    </div>
                  ...
                  <div className="cart-item-actions">
                    <button type="button" className="cart-action" onClick={() => handleRemoveSaved(item)}>
                      <MdDeleteOutline /> Remove
                    </button>
                    <button
                      type="button"
      ...
      {items.length > 0 && (
        <>
          {/* Fixed footer: delivery strip stacked above the checkout summary */}
                ...
                <div className="cart-delivery-progress cart-delivery-complete">
                  <span className="cart-delivery-icon">
                    <img
                      ...
                      onError={(e) => {
                        console.error('fastdelivery.png failed to load:', fastDeliveryIcon);
              ...
              <div className="cart-summary">
                <div className="cart-summary-left">
                  <p className="cart-savings-line">
                    <MdAutorenew size={15} /> You will save ₹{totalSavings.toLocaleString('en-IN')} on this order
                  </p>
                  <button
                    type="button"
                  ...
                  >
                    View price details
                    <MdKeyboardArrowDown className={priceDetailsOpen ? 'open' : ''} />
                  </button>
                  {priceDetailsOpen && (
                    <div className="cart-price-details">
                      ...
                      <div>
                        <span>Delivery</span>
                        <span>{amountToFreeDelivery > 0 ? 'Calculated at checkout' : 'FREE'}</span>
                      </div>
                    </div>
                  )}
                </div>
