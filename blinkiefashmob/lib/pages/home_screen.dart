...
class _HomeScreenState extends State<HomeScreen>
  ...
  static const LinearGradient _homeBgGradient = LinearGradient(
    ...
    colors: [Color(0xFFFAFAFA), Color(0xFFFAFAFA), Color(0xFFFAFAFA)],
  );
  ...
  // Initialize and update daily deals timer, refreshing products at midnight
  void _startDealsTimer() {
    _updateDealsTimer();
    // Update timer every second
    _dealsTimer = Timer.periodic(const Duration(seconds: 1), (_) {
      _updateDealsTimer();
    });
  }

  void _updateDealsTimer() {
    final now = DateTime.now();
    final today = DateTime(now.year, now.month, now.day);
    final tomorrow = today.add(const Duration(days: 1));
    final secondsLeft = tomorrow.difference(now).inSeconds;

    final hours = (secondsLeft / 3600).floor();
    final minutes = ((secondsLeft % 3600) / 60).floor();
    final seconds = secondsLeft % 60;

    // Refresh deals if date changed
    if (_lastDealsRefreshDate != today) {
      _lastDealsRefreshDate = today;
    }
  ...
  Future<void> _loadHomeData({double? lat, double? lng}) async {
    try {
      ...
      unawaited(_loadRecentlyExploredProducts());
    } catch (_) {
      if (mounted) setState(() => _isLoading = false);
    }
  }

  Future<void> _loadCollectionProducts({
    required String rootCategoryName,
    required void Function(List<Map<String, dynamic>>) assign,
    ...
    try {
      ...
      final rootCatId = root['id']?.toString();
      ...
      final products = (result['products'] as List? ?? [])
          ...
          .toList();
      if (mounted) {
        setState(() => assign(products));
      }
    } catch (_) {}
  }

  Future<void> _loadMensProducts() async {
    await _loadCollectionProducts(
      rootCategoryName: 'Men',
      assign: (products) => _mensProducts = products,
    );
  }

  Future<void> _loadWomensProducts() async {
    await _loadCollectionProducts(
      rootCategoryName: 'Women',
      assign: (products) => _womensProducts = products,
    );
  }

  Future<void> _loadKidsProducts() async {
    await _loadCollectionProducts(
      rootCategoryName: 'Kids',
      assign: (products) => _kidsProducts = products,
    );
  }
  ...
  Future<void> _detectCurrentLocation() async {
    try {
      ...
      debugPrint('[LOC] Starting fresh GPS detection (15s timeout)...');
      ...
      if (mounted && _lastKnownLat == null) {
        setState(() => _currentLocation = 'Set location');
      }
    } catch (e) {
  ...
  Future<void> _reverseGeocodeAndUpdate(
    ...
    try {
      ...
      if (city.isNotEmpty && mounted) {
        ...
        if (persist) {
          _persistResolvedLocation(city: city, lat: lat, lng: lng);
        }
    ...
    return AppBar(
      ...
      title: Padding(
        ...
        child: Column(
          ...
          mainAxisSize: MainAxisSize.min,
          children: [
            // First row: Logo on left, Location selector on right
            Row(
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        ConstrainedBox(
                          ...
                          child: Text(
                            'Delivering to $_currentLocation',
                            maxLines: 1,
            ...
            // Second row: Search bar on left, Icons on right
            Row(
              children: [
                const Expanded(child: AnimatedSearchBar(bottomPadding: 0)),
  ...
  Widget _expressBody() {
    if (_isLoading) {
      return Container(
        ...
        child: const BfPageLoader(message: 'Finding express deals near you...'),
      );
    }
  ...
  Widget _expressUnavailableBanner() {
    return Container(
    ...
    return Padding(
      ...
      child: Column(
        children: [
          ...
          // ── Tagline ──────────────────────────────────────────────────
          const Text(
            'Top Brands  ·  Latest Styles  ·  Handpicked for You.',
            textAlign: TextAlign.center,
            style: TextStyle(
              fontSize: 12,
  ...
  // ── Brand Banners Grid ────────────────────────────────────────────────────
  Widget _brandBannersGrid() {
    const r = BorderRadius.all(Radius.circular(14));

    Widget panel(String asset, VoidCallback onTap) => GestureDetector(
      onTap: onTap,
      child: ClipRRect(
        borderRadius: r,
        child: Image.asset(
    ...
    return Padding(
      ...
      child: Column(
        children: [
          ...
          const SizedBox(height: 8),
          // Row 2 – FCUK & French Connection (full width)
          panel(
            'assets/images/FcukandFrenchconnection.jpeg',
  ...
  Widget _dealsOfTheDayWithTimerSection() {
    return Column(
      children: [
        Padding(
          padding: const EdgeInsets.fromLTRB(12, 16, 12, 8),
              ...
              child: Column(
                ...
                children: [
                  Container(
                    ...
                    height: 70,
                    decoration: BoxDecoration(
                      color: tileBackgrounds[i % tileBackgrounds.length],
  ...
  // ── Men's Creative Section ────────────────────────────────────────────────
  static const _mensCats = [
  ...
  static const _womensCats = [
    {'emoji': '👗', 'label': 'Dresses'},
    {'emoji': '🧵', 'label': 'Kurtis'},
    {'emoji': '👜', 'label': 'Handbags'},
    {'emoji': '👠', 'label': 'Heels'},
    {'emoji': '🩴', 'label': 'Flats'},
  ...
  static const _kidsCats = [
    {'emoji': '👶', 'label': 'Baby'},
    {'emoji': '👕', 'label': 'T-Shirts'},
    {'emoji': '🩳', 'label': 'Shorts'},
    {'emoji': '👟', 'label': 'Shoes'},
    {'emoji': '🎒', 'label': 'School Bags'},
    {'emoji': '🧢', 'label': 'Caps'},
    {'emoji': '🧸', 'label': 'Toys'},
    {'emoji': '🧦', 'label': 'Essentials'},
  ];

  static const _electronicsCats = [
  ...
  Widget _mensCreativeSection() {
    return _collectionCreativeSection(
      audienceLabel: 'Men',
      products: _mensProducts,
      categoryChips: _mensCats,
    );
  }

  Widget _collectionCreativeSection({
  ...
  List<Map<String, dynamic>> _topDiscountedProducts({
    ...
    for (final product in filteredProducts) {
      ...
      if (price > discPrice && price > 0) {
        ...
        if (minDiscount == null || discount > minDiscount) {
          final brandName = (product['brand']?.toString() ?? '').toLowerCase();
          final isPremiumBrand = premiumBrands.any(
            (brand) => brandName.contains(brand),
          );
          dealsProducts.add({
            ...product,
    ...
    // Sort: premium brands first (by discount), then others (by discount)
    dealsProducts.sort((a, b) {
      final isPremiumA = a['is_premium_brand'] as bool? ?? false;
      final isPremiumB = b['is_premium_brand'] as bool? ?? false;
      ...
      // Within same category, sort by discount (highest first)
  ...
  Widget _buildRecentlyExploredSection() {
    if (_recentlyExploredProducts.isEmpty) {
  ...
  // ── Coupon Banner ─────────────────────────────────────────────────────────
  Widget _couponBanner() {
    return GestureDetector(
      onTap: () async {
        const playStoreUrl =
            'https://play.google.com/store/apps/details?id=com.blinkiefash.app';
        try {
  ...
  Widget _shopByBrands() {
    ...
    // Priority brands order (same as web version)
    ...
    sortedBrands.sort((a, b) {
      ...
      // Non-priority brands sorted alphabetically
